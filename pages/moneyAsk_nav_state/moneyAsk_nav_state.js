...
Page({
    ...
    getdata: function (id) {
        let that = this
        var url = app.globalData.URL + '/oaprojectbonus/getOaProjectBonusAuditDetailByBonusId?processInstanceId='+id;
        wx.request({
            url: url,
            method: 'POST',
              ...
              if (data.length != 0) {
                setTimeout(function () {
                  for(let i=0;i<data.length;i++) {
                      ...
                      obj.state = data[i].auditStatus
                      // obj.statename = data[i].status=='通过'?'通过':'待审核'
                      obj.zhiwei = data[i].positionName
                      obj.remark = data[i].auditOpinion
                      obj.time = data[i].auditTime!=null?app.formatDate(data[i].auditTime):''
                      list.push(obj)
                  }
                  that.setData({
                    list: list
    ...
    onLoad: function (options) {
       // console.log(options.id)
        this.getdata(options.processInstanceId)

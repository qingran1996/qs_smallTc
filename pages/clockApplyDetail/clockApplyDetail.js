...
Page({
    ...
    onLoad: function (options) {
        var clockId = options.clockId;
        this.init2(clockId);
        wx.showLoading({title: '加载中'});
    },
    init2: function (clockId) {
        var _this = this;
        app.data.qs.get_clock_flow({
            'clock_id': clockId,
            success: function (res) {
                if (res.code == 200) {
                    ...
                    var canReSubmit = res.data['rights']['can_re_submit'];
    ...
    confirm_rollback: function () {
        ...
        app.data.qs.rollback_clock({
            'flow_run_id': _this.data.runId,
            success: function (res) {
                app.data.qs.showSuccessToast(res.msg);
                wx.redirectTo({
                    url: '../clockApplyDetail/clockApplyDetail?clockId=' + _this.data.clockId
                });
            },
            fail: function (res) {

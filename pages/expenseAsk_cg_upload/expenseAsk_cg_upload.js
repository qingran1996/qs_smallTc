...
Page({
  ...
  saveTo: function (e) {
    if (this.data.moneyNum == '') {
      ...
      let that = this
      var url = app.globalData.URL + '/expenses/expensesPurchaseAdd';
  ...
  deletemoney:function() {
    let that = this
    var url = app.globalData.URL + '/expenses/expensesPurchaseDeleteById';
    // let form1 = new FormData()
    // form1.append('file',that.data.partwordpath)
    wx.request({
      url: url,
  ...
  getnav: function (id) {
    let that = this
    var url = app.globalData.URL + '/expenses/expensesPurchaseGetById';
    wx.request({
      url: url,
      method: 'POST',
      data: {
